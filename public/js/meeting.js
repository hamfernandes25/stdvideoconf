...
var Meeting = function (socketioHost) { 
	 ...
	 */   
    function joinRoom(name,id,check,uname) {
        _room = name;
        ...
        if (_room !== '') {
            ...
            _defaultChannel.emit('create or join', {room:_room, from:_myID,prof:check});
        }
        ...
        window.onbeforeunload = function(e){
            ...
            _defaultChannel.emit('message',{type: 'bye', from:_myID,prof:_check});
    ...
    function goodbye(uid){
        ...
        _defaultChannel.emit('message',{type: 'bye', from:uid,prof:_check});
    ...
    function initDefaultChannel() {
        ...
        _defaultChannel.on('message', function (message){
            ...
            if (message.type === 'newparticipant') {
                ...
                createOffer(partID,check,name1);
            ...
            if (message.dest===_myID) {
                if(message.type === 'offer') {
                  ...
                  var name=message.name;
                    createAnswer(message, _privateAnswerChannel, to,check,name);
	 ...
	 *
	 */
    function addRemoteVideo(stream, from,vcheck,vname) {
        // call the callback
        _onRemoteVideoCallback(stream, from,vcheck,vname);
    }
	 ...
	 */
    function createOffer(participantId,check,name) {
        console.log('Creating offer for peer '+participantId+"  "+check+name);
        ...
        _opc[participantId].onaddstream = handleRemoteStreamAdded(participantId,check,name);
        ...
        var onSuccess = function(participantId) {
            return function(sessionDescription) {
               ...
               channel.emit('message', {snDescription: sessionDescription, from:_myID, type:'offer', dest:participantId,check:_check,name:_name});        
    ...
    function createAnswer(sdp, cnl, to,check,name) {
        ...
        _apc[to].onaddstream = handleRemoteStreamAdded(to,check,name);
...
//console.log(_opc);
    ...
    function handleUserMedia(stream) {
        ...
        _localStream = stream;
        _defaultChannel.emit('message', {type:'newparticipant', from: _myID,check:_check,name:_name});
    }
    ...
    function handleRemoteStreamAdded(from,check,name) {
	    return function(event) {
        	console.log('Remote stream added');
			addRemoteVideo(event.stream, from,check,name);
